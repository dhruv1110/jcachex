...
public class FIFOEvictionStrategy<K, V> implements EvictionStrategy<K, V> {
    ...
    public K selectEvictionCandidate(Map<K, CacheEntry<V>> entries) {
        readLock.lock();
        try {
            // Return the oldest entry (tail of the list)
            InsertionNode<K> oldest = tail.prev;
            return (oldest != head) ? oldest.key : null;
        } finally {
            readLock.unlock();
        }
    }
    ...
    public void update(K key, CacheEntry<V> entry) {
        ...
        try {
            ...
            // For FIFO, we don't update position on access - only on insertion
        } finally {
            writeLock.unlock();
        }
    }
